...
1. **Identify our target genes in all of our input genomes**
  * these are represented by the colors in our cartoon on the right, notice that Genome "B" is missing one of them

2. **Align each set of identified genes individually**
  * e.g. all the copies of target gene "X" that were identified from all input genomes are aligned together; all the copies of target gene "Y" that were identified from all input genomes are aligned together; and so on
...
3. **Stick all of these alignments together horizontally**
  ...
  * the total number of sequences of the combined alignment will be the total number of input genomes (ignoring if any input genomes were removed for some reason)
